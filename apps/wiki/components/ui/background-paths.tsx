...
function FloatingPaths({ position }: { position: number }) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      <svg className="h-full w-full text-slate-900 opacity-80 dark:text-white" viewBox="0 0 696 316" fill="none">
        ...
        {paths.map((path) => (
          <motion.path
            ...
            strokeOpacity={Math.min(0.14 + path.id * 0.018, 0.8)}
...
export function BackgroundPaths({
  ...
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-transparent">
      {showBackdrop ? <BackgroundPathsBackdrop /> : null}
